select 
    ...
    split(full_contact_name, ' ')[OFFSET(0)] as first_name,
    split(full_contact_name, ' ')[OFFSET(0)] as last_name,
    case 
        when `Primary Caregiver` like '%Primary Caregiver%' then 'Primary Caregiver'
        when `Primary Caregiver` like '%Director%' then 'Director'
        else null
    end as title,
    ...
    null as county,
    'US' as country,
    concat("1", replace(replace(replace(`Phone`, '-', ''), '(',''), ')', '')) as phone1,
    concat("1", replace(replace(replace(`Phone`, '-', ''), '(',''), ')', '')) as mobile_phone,
    `Email` as email,
    null as website,
    ...
    null as provider_id,
    case when `Accepts Subsidy` like '%Accepts Subsidy%' then 'Y' else 'N' end as accepts_financial_aid,
    concat(`Ages Accepted 1`, CHAR(10), `AA2`, CHAR(10), `AA3`, CHAR(10), `AA4`) as ages_served,
    ...
    case when `AA4` is null then 
        case when `AA3` is null then 
